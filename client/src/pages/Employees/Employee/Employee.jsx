    ...
    return (
        <div>
            <div style={{display: 'flex', alignItems: 'center'}}>
                <Button onClick={onClick} size={'large'} variant={'outlined'} sx={{padding: '7px', minWidth: '36px'}}>
                    <SvgIcon>
                        <ArrowBackOutlinedIcon/>
                    </SvgIcon>
                </Button>
                <div style={{display: 'flex', justifyContent: 'center', width: '100%', marginRight: '40px'}}>
                    <Typography variant={'h4'}>{name}</Typography>
                </div>
            </div>
            <div style={{marginTop: '20px'}}>
                <StyledTabs scrollButtons={false} value={type} centered={true}>
                    <Tab component={RouterLink} to={`/employees/${id}/profile`} label={t('Profile')} value={'profile'}/>
                    <Tab component={RouterLink} to={`/employees/${id}/leaves`} label={t('Leaves')} value={'leaves'}/>
                </StyledTabs>
