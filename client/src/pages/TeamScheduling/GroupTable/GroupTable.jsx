...
const GroupTable = ({daysOfMounth, daysInMonth, currentYearUserLeaves}) => {
    ...
    const onToggle = useCallback((department) => {
            ...
            <TableContainer className={classes.container}>
                <Table stickyHeader aria-label="simple table">
                    <TableHead style={{height: '68px'}}>
                        <TableRow>
                            <TableCell align={'center'} className={classes.departmentCellName}>{t('Departments')}</TableCell>
                            {daysOfMounth.map(day => (
                                <TableCell align={'center'} className={classes.cell}
                                           key={day}>{t(format(day, 'EEEEEE'))}<br/>{getDate(day)}</TableCell>
                            ))}
                        </TableRow>
                    </TableHead>
                            ...
                            <TableRow key={department.name} className={classes.departmentRow}>
                                {daysInMonth.map(day => day ?
                                    ...
                                    <TableCell key={day} className={classes.departmentCell}
                                        ...
                                        <Stack direction="row" alignItems="center" gap={1} className={classes.chip}
                                               sx={{
                                                   background: alpha(department.color, 0.1),
                                                   color: department.color
                                               }}>
                                            <Typography variant="body1">{department.name}</Typography>
                                            {toggle[department.name] ? <ExpandLessIcon/> :
                                                <ExpandMoreIcon/>}
                                        </Stack>
                                    </TableCell>
                                )}
                                        ...
                                        <TableRow key={user.name}>
                                            {daysInMonth.map(day => (
                                                <TableCell key={day} className={classes.cell}>
                                                    {day ? user.userLeave.map(leave => {
                                                        if (day >= getDate(new Date(leave.startDate)) && day <= getDate(new Date(leave.endDate)))
                                                            return (<Tooltip key={leave.leaveType.name}
                                                                             title={t(leave.leaveType.name)}>
                                                                <div className={classes.circle}
                                                                     style={{background: alpha(leave.leaveType.color, 0.3), border: `1px solid ${leave.leaveType.color}`}}/>
                                                            </Tooltip>)
                                                    }) : user.name}
                                                </TableCell>
                                            ))}
