...
const ReportsTable = ({year}) => {
    ...
    const leaveTypes = dataLeaveTypes?.getLeaveTypes?.filter((item) => item.name !== 'Public Holiday') || []
    ...
    const currentYearUserLeaves = allUsersLeaves.map(({userLeave, ...rest}) => {
        const userLeaveFiltered = userLeave.filter(({
                                                        startDate,
                                                        status
                                                    }) => status === 'accepted' && year === getYear(new Date(startDate)))
        return {
    ...
    const gridColumns = leaveTypes.map((leaveType, index) => ({
        ...
        renderHeader: ({colDef}) => {
            return (
                <Status name={colDef.headerName} color={colDef.color}/>
            )
        }
    }));
