...
export default function NotificationsPopover() {
    ...
    const unreadNotifications = notifications?.filter(({read}) => read?.filter((item) => item.reader._id !== id).length === read.length)
    const readNotifications = notifications?.filter(({read}) => read?.find((item) => item.reader._id === id))
    ...
    return (
        <>
            ...
            >
                <Box sx={{display: 'flex', alignItems: 'center', py: 2, px: 2.5}}>
                    <Box sx={{flexGrow: 1}}>
                        ...
                        <Typography variant="body2" sx={{color: 'text.secondary'}}>
                            {t('You have {{count}} unread message', {count: unread})}
                        </Typography>
                    </Box>
                ...
                <Paper sx={{maxHeight: {xs: 340, sm: 'auto'}, overflow: 'auto', backgroundColor: 'background.default'}}
                       elevation={0}>
                    <List
                        disablePadding
                        subheader={
                    ...
                    >
                        {unreadNotifications.map((notification) => (
                            <NotificationItem key={notification._id} notification={notification} style={{backgroundColor: 'action.selected'}}/>
                        ))}
                    </List>
                    ...
                    <List
                        disablePadding
                        subheader={
                            <ListSubheader disableSticky sx={{py: 1, px: 2.5, typography: 'overline', borderTop: (theme) => theme.palette.mode === 'light' ? '1px dashed #bfc0c1' : '1px dashed #4c5057',
                            }}>
                                {t('Before that')}
                            </ListSubheader>
                        }
    ...
    return (
        <ListItem
            sx={{
                py: 1.5,
                px: 2.5,
                borderTop: (theme) => theme.palette.mode === 'light' ? '1px solid #bfc0c1' : '1px solid #4c5057',
                ...style
            }}
        >
            <ListItemText
