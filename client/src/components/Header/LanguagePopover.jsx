...
const LanguagePopover = () => {
    ...
    return (
        <>
            ...
            >
                <Avatar src={languages[lng === 'en' ? 0 : 1].icon} alt="language" sx={{width: '24px', height: '24px'}}/>
            </IconButton>
            ...
            >
                <Stack spacing={0.75}>
                    {languages.map((option) => (
                        <MenuItem key={option.value} selected={option.value === lng} onClick={() => handleChange(option.value)}>
                            <Box component="img" alt={option.label} src={option.icon} sx={{ width: 28, mr: 2, borderRadius: '50%' }} />
                            {option.label}
                        </MenuItem>
                    ))}
                </Stack>
